					 ...
					 struct gpio_keys_button_data *bdata)
{
	...
	if (state != bdata->last_state) {
		...
		bdata->long_pressed = 0;
	}
	else if((state2==1)&&(state==1))
	{
		struct bh_priv *priv = &bdata->bh;
		unsigned long seen = jiffies;
		unsigned int type = button->type ?: EV_KEY;
		unsigned long tmp = ((seen - priv->seen) / HZ);
		
		if((bdata->long_pressed !=1)&&(tmp == 5)){
			button_hotplug_event(bdata, type, button->code, 2);
			bdata->long_pressed = 1;
		}
