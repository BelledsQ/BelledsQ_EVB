...
mac80211_hostapd_setup_bss() {
	...
	#kapple modify [ "$staidx" -gt 0 ] && append hostapd_cfg "start_disabled=1" "$N"
...
detect_mac80211() {
	...
	for dev in $(ls /sys/class/ieee80211); do
		...
		macaddr="$(cat /sys/class/ieee80211/${dev}/macaddress |sed 's/://g'|cut -c 7-12 | tr [a-z] [A-Z])"
		if [ ! -f /etc/config/airplay ]; then
			[ ! -z $macaddr ] &&  echo -e "\nconfig bellmusic${macaddr} 'unix'\n" > /etc/config/airplay
		fi
